...
export class LocalOrderBook {
  ...
  apply(message, recvMs = Date.now()) {
    ...
    const changed = this.#applySide(this.bids, data.b) + this.#applySide(this.asks, data.a);
    this.ready = true;
    this.dirty = true;
    this.version += 1;
    this.lastChangedLevels = changed;
    this.last = {
      recvMs,
  ...
  metrics(n = 10) {
    if (!this.ready) return { bookValid: 0, mid: null, spreadBps: null, bidDepth: null, askDepth: null, imbalance: null };
    const { bids, asks } = this.top(n);
    if (!bids.length || !asks.length) return { bookValid: 0, mid: null, spreadBps: null, bidDepth: null, askDepth: null, imbalance: null };
  ...
  snapshot(sampleTsMs = Date.now()) {
    ...
    const { bids, asks } = this.top(this.depth);
