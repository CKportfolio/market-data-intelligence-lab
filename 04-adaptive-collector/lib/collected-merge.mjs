...
export async function discoverCollectedSources(rawDir) {
  ...
  // Legacy staging plus namespaced staging-adaptive used for zero-gap handoff.
  const rawChildren = await listDirs(rawDir);
  const stagingRoots = rawChildren.filter((x) => /^staging(?:-|$)/.test(path.basename(x)));
  for (const stagingRoot of stagingRoots) {
    for (const dir of await listDirs(stagingRoot)) {
  ...
  // Include legacy live/current and new live/current-adaptive (or any future current-* namespace).
  for (const currentDir of await listDirs(path.join(rawDir, "live"))) {
    if (!/^current(?:-|$)/.test(path.basename(currentDir))) continue;
    const currentFile = path.join(currentDir, "market.jsonl");
    const currentMeta = path.join(currentDir, "batch.json");
    ...
    if (inspected?.startMs != null) {
      ...
      result.push({ type: "current", path: currentFile, filePath: currentFile, batchId: meta.batchId || path.basename(currentDir), ...inspected });
    }
  }
