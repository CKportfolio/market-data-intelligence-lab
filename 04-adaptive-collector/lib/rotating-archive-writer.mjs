...
export class RotatingArchiveWriter {
  constructor(baseDir, { maxBytes = 900 * 1024 * 1024, onArchive = null, liveName = "current", collectorId = null } = {}) {
    this.baseDir = baseDir;
    this.maxBytes = maxBytes;
    this.onArchive = onArchive;
    this.liveName = liveName || "current";
    this.collectorId = collectorId || this.liveName;
    const suffix = this.liveName === "current" ? "" : `-${this.liveName.replace(/^current-?/, "").replace(/[^a-zA-Z0-9_-]/g, "_") || "adaptive"}`;
    this.liveDir = path.join(baseDir, "live");
    this.currentDir = path.join(this.liveDir, this.liveName);
    this.stagingDir = path.join(baseDir, `staging${suffix}`);
  ...
  async #writeNow(channel, rows, fallbackTs) {
    const normalized = rows.map((row) => {
      ...
      return { ...row, _channel: channel, tsRecordMs: ts, _collectorId: this.collectorId };
    });
